...
def get_grad(x, disc_interpolates):
    ...
    def train(x, y, grad_real_ema, grad_fake_ema):
        ...
        for step_index in range(config['num_D_steps']):
            ...
            for accumulation_index in range(config['num_D_accumulations']):
                ...
                if step_index==0 and config['use_DigGAN']:                    
                    grads = get_grad(D_input, D_out)
                    grad_fake, grad_real = torch.split(grads, [config['batch_size'], config['batch_size']])
                    if grad_real_ema is not None:
                        grad_real = 0.5 * grad_real + 0.5 * grad_real_ema
                        grad_fake = 0.5 * grad_fake + 0.5 * grad_fake_ema
                    D_loss = D_loss + ((grad_fake-grad_real)**2).mean() * config['DigGAN_penalty_weight']
