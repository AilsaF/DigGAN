...
class make_worker(object):
    def __init__(self, cfgs, train_configs, model_configs, run_name, best_step, logger, writer, n_gpus, gen_model, dis_model,
        ...
        self.diggan_penalty_for_dis = cfgs.diggan_penalty_for_dis
        self.diggan_penalty_lambda = cfgs.diggan_penalty_lambda
        self.cr = cfgs.cr
        ...
        if self.dataset_name == "imagenet":
            ...
            self.num_eval = {'train':5994, 'valid':5794}
        else:
        ...
        self.grad_real_ema = None
        self.grad_fake_ema = None
        self.ema_losses = ema_losses(start_itr=1000)
            ...
            for step_index in range(self.d_steps_per_iter):
                ...
                for acml_index in range(self.accumulation_steps):
                    ...
                    with torch.cuda.amp.autocast() if self.mixed_precision else dummy_context_mgr() as mpc:
                        ...
                        if step_count%4==0 and self.diggan_penalty_for_dis:
                            grad_real = calc_derv(real_images, real_labels, self.dis_model, self.conditional_strategy, self.local_rank)
                            grad_fake = calc_derv(fake_images, fake_labels, self.dis_model, self.conditional_strategy, self.local_rank)
    ...
    def run_image_visualization(self, nrow, ncol, standing_statistics, standing_step):
        self.sampler = 44
        # self.sampler = torch.tensor(list(range(9*self.batch_size//4+1, 10*self.batch_size//4+1))*4).to('cuda')
        # self.sampler = torch.tensor(list(range(2*self.batch_size//8+1, 3*self.batch_size//8+1))*8).to('cuda')
        # self.sampler = torch.tensor(range(32)).to('cuda')
        # print(self.sampler)
        ...
        with torch.no_grad() if self.latent_op is False else dummy_context_mgr() as mpc:
            ...
            plot_img_canvas((generated_images.detach().cpu()+1)/2, "./figures/{run_name}/generated_canvas{ind}.png".\
                            format(run_name=self.run_name, ind=self.sampler), ncol, self.logger, logging=True)
